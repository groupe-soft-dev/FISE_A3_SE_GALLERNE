...
namespace EasySaveV3.AppClient.ViewModel
{
    public class BackUpViewModel : ViewModelBase, INotifyPropertyChanged
    {
        ...
        public BackUpViewModel(INavigationService navigation, IMessageBoxNotifier notifier, SocketClientService socket) 
            : base(socket)
        {
        ...
        public void LoadFromExistingJob(Core.Model.BackUpJob job)
        {
            _job = job;
            Id = job.Id;
            Name = job.Name;
        ...
        private void CreateJob()
        {
            if (IsValid())
            {
                try
                {
                    _job = new Core.Model.BackUpJob(Name, dirSource, dirTarget, Differential, Encryption);
                    _job.CreateJob();
